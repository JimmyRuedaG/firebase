...
function cargarDatos(idRestaurante) {
    ...
    const docRef = doc(db, "resturante", idRestaurante);
    ...
    Promise.all(promises).then(values =>{
        ...
        if(flagSuccess) {
            ...
            Promise.all(promises2).then(values =>{
                ...
                const restauranteRef = doc(db, "resturante", idRestauranteGolbal);
...
function guardar() {
    ...
    const newRestaurant = doc(collection(db, "resturante"));
       ...
       Promise.all([uploadTask, uploadTaskFile]).then(values =>{
            ...
            if(flagSuccess) {
                ...
                Promise.all([downloadUrlFoto, downloadUrlPdf]).then(valuesUpload =>{
                        ...
                        const resturanteRef = doc(db, "resturante", newRestaurant.id);
...
window.eliminar =  function eliminar(idRestaurante) {
     ...
     /*deleteDoc(doc(db, "resturante", idRestaurante)).then(() =>{
            alert("Eliminado correctamente");
            cargarRestaurantes();
     }).catch((error)=>{
            alert("Ocurrio un error al eliminar");
     });*/
     const restauranteRef = doc(db, "resturante", idRestaurante);
     updateDoc(restauranteRef, {
         visible:  false
     }).then(() =>{
        alert("Eliminado correctamente");
